import os, sys, shutil, re, glob, subprocess

if sys.platform == "win32":
    sys.stdout.reconfigure(encoding='utf-8')

ROOT_DIR = r'c:\Users\31085\Desktop\红宝书讲义_排版修复版\_内部核心程序与数据(无需修改)'
BG_FILE = os.path.join(ROOT_DIR, 'build_golden_datasets.py')
BAK_FILE = os.path.join(ROOT_DIR, 'build_golden_datasets.py.bak')

if not os.path.exists(BAK_FILE):
    shutil.copy2(BG_FILE, BAK_FILE)
    print(f"[✓] Created backup: {BAK_FILE}")

with open(BG_FILE, 'r', encoding='utf-8') as f:
    code = f.read()

# We will inject is_word_header_robust and update build_list_words_perfect in build_golden_datasets.py
# Let's inspect where extract_section_content is defined and update it
new_extract_func = '''def clean_txt(text):
    if not text: return ""
...
def extract_section_content(block, start_patterns, end_patterns):
    """
    通用、稳健的区间文本抽取器：
    从 start_patterns 匹配的行开始，截取到 end_patterns 出现为止。
    免疫各种 Markdown 加粗星号转义、冒号内外置、换行折叠。
    """
...
# Find old clean_txt / extract_section_content and replace
old_extract_pat = r'def clean_txt\(text\):.*?def find_batch_file\(l_num, b_num\):'
match_extract = re.search(old_extract_pat, code, re.DOTALL)
if match_extract:
    code = code[:match_extract.start()] + new_extract_func + '\ndef find_batch_file(l_num, b_num):' + code[match_extract.end():]
    print("[✓] Replaced clean_txt and extract_section_content with robust versions")
else:
    print("[!] Could not match extract_section_content pattern")

# Now replace build_list_words_perfect
new_build_func = '''def build_list_words_perfect(l_num, mineru_official=None):
    ...
    for b_num in range(1, 5):
        ...
        # 定位词汇在批次 Markdown 中的标题坐标（严防属性子项与跨词串扰）
        positions = []
        ...
        for no_int in range((b_num-1)*15+1, b_num*15+1):
            ...
            if not is_writing:
                ...
                # Check inline translation in en e.g. (教育在引导...)
                if en and not zh:
    ...
    return guard_and_enrich_words(words_data, verbose=True)
'''

old_build_pat = r'def build_list_words_perfect\(l_num, mineru_official=None\):.*?def build_all_lists\(\):'
match_build = re.search(old_build_pat, code, re.DOTALL)
if match_build:
    code = code[:match_build.start()] + new_build_func + '\ndef build_all_lists():' + code[match_build.end():]
    print("[✓] Replaced build_list_words_perfect with robust version")
else:
    print("[!] Could not match build_list_words_perfect pattern")

# Write updated code back
with open(BG_FILE, 'w', encoding='utf-8') as f:
    f.write(code)
print(f"[✓] Successfully patched {BG_FILE}")
